...
namespace MedicalAssociationAPI.Filters
{
                ...
                if (_isCacheable(ac))
                {
                    ...
                    if (WebApiCache.Contains(_cachekey))
                    {
                        ...
                        if (val != null)
                        {
                            ...
                            ac.Response = request.CreateResponse();
    ...
    public static class CachingData
    {
        ...
        public static object GetDataFromCache(string cacheKey)
        {
            return WebApiCache.GetCacheItem(cacheKey);
        }

        public static void AddCache(string cacheKey, object body)
        {
            var cachePolicty = new CacheItemPolicy();
            cachePolicty.AbsoluteExpiration = DateTime.Now.AddDays(30);
            WebApiCache.Add(cacheKey, body, cachePolicty);
